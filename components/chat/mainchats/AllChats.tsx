...
import { FaSearch, FaArrowLeft, FaUserPlus } from "react-icons/fa"; // Import icons for search and back
        ...
        {filteredChats.map(
          (
            ...
            return (
              <div
                key={chat._id}
                onClick={() => handleChatClick(chat._id, otherUser?._id || "")}
                className="flex items-center p-2 sm:p-3 hover:bg-gradient-to-br active:scale-6 hover:from-neutral-300/30 hover:to-yellow-200/10 hover:via-gray-300/10 active:scale-95 active:opacity-80 rounded-lg transition-all duration-75 ease-in-out cursor-pointer mb-2"
                style={{
                  animation: `fadeIn 0.5s ease-in-out ${index * 0.1}s forwards`,
                  opacity: 0,
                  // animationFillMode: "forwards", // Add this line
                }}
              >
                {/* User Avatar */}
