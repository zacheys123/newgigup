...
import SchedulerComponent from "@/components/gig/create/SchedulerComponent";
import {
  BusinessCategory,
  CategoryVisibility,
  CustomProps,
  ...
  TalentType,
  UserInfo,
} from "@/components/gig/create/types";
import { cn } from "@/lib/utils";
import TalentModal from "@/components/gig/create/TalentModal";
...
const EditPage = () => {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    alert("schedulingProcedure.type");
    if (!validateFields()) {
      setIsVisible(true);
      setError(true);
    }
  ...
  const sectionBgClasses = {
    business: "bg-gray-800/40 border-l-4 border-l-blue-500",
    title: "bg-gray-800/40 border-l-4 border-l-amber-500",
    description: "bg-gray-800/40 border-l-4 border-l-emerald-500",
    timeline: "bg-gray-800/40 border-l-4 border-l-purple-500",
    band: "bg-gray-800/40 border-l-4 border-l-rose-500",
    duration: "bg-gray-800/40 border-l-4 border-l-red-500",
    default: "bg-gray-800/40 border-l-4 border-l-gray-500",
  };
  const inputVariants = {
    focus: {
      scale: 1.02,
  ...
  const [open, setShowOpen] = useState(true);
  const handleClose = () => {
    router.back();
    setShowOpen(false);
  };
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="h-[90vh] sm:h-[88vh] mt-0 sm:mt-[20px] py-2 sm:py-3 overflow-y-auto bg-gray-900 w-full sm:w-[90%] mx-auto px-2 sm:px-4 backdrop-blur-lg bg-opacity-50">
        {" "}
        <AnimatePresence>
        ...
        <div className="w-[90%] h-full mx-auto px-4 py-3 md:py-8 overflow-y-auto">
          ...
          >
            <DialogTitle className="text-gray-400 font-light my-2">
              Update Your Gig
            </DialogTitle>
          </motion.div>
